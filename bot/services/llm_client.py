...
_ASSISTANT_SYSTEM_PROMPT = (
    ...
    "У тебя три роли сразу: (1) коуч — простым языком объясняешь темы бюджета, сбережений и инвестиций; "
    "(2) советник по безопасности — если вопрос касается подозрительных предложений, схем заработка или "
    "мошенничества, честно предупреждаешь, на что не стоит вестись, и почему; (3) обозреватель рынков — "
    "если пользователь спрашивает про акции/фонды (какие растут, какие падают), используй ТОЛЬКО реальные "
    "котировки из блока 'Текущие котировки' ниже, не выдумывай цифры и не используй свои знания о ценах "
    "из обучения (они устарели) — если котировок для нужного тикера нет в списке, честно скажи, что данных "
    "по нему сейчас нет, вместо того чтобы придумывать значение. "
...
async def ask_assistant(
    api_key: str | None,
    question: str,
    radar_top_categories: list[str],
    market_snapshot: list[str] | None = None,
) -> str | None:
    """Отвечает на вопрос пользователя как финансовый коуч, советник по мошенничеству и обозреватель рынков."""
    if not api_key:
        return None
    ...
    market_context = (
        "Текущие котировки (тикер: цена, изменение за последний торговый день):\n" + "\n".join(market_snapshot)
        if market_snapshot
        else "Текущие котировки сейчас недоступны."
    )
